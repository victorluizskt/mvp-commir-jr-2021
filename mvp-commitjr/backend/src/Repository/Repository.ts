...
async function getMovieGenre(genre:string){
    try {
        const response = await api.get(`https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&language=en-US&year=2021&with_genres=${genre}`);
...
export {getPersonId, getGenresList, getMovieGenre};